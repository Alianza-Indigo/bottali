...
export async function createConversation(
  ...
  const [conversation] = await db
    ...
    .values({ organizationId, userId, toolId, toolVersionId })
    .returning();
...
export async function getConversationWithMessages(
  ...
  const conversation = await getOwnedConversation(conversationId, userId, organizationId);
...
export async function renameConversation(
  ...
  await getOwnedConversation(conversationId, userId, organizationId);
  await db.update(conversations).set({ title, updatedAt: new Date() }).where(eq(conversations.id, conversationId));
}

export async function archiveConversation(conversationId: string, userId: string, organizationId = DEFAULT_ORGANIZATION_ID) {
  await getOwnedConversation(conversationId, userId, organizationId);
  await db.update(conversations).set({ status: "ARCHIVED", archivedAt: new Date() }).where(eq(conversations.id, conversationId));
}

export async function restoreConversation(conversationId: string, userId: string, organizationId = DEFAULT_ORGANIZATION_ID) {
  await getOwnedConversation(conversationId, userId, organizationId);
...
export async function deleteConversation(conversationId: string, userId: string, organizationId = DEFAULT_ORGANIZATION_ID) {
  await getOwnedConversation(conversationId, userId, organizationId);
...
export async function clearConversationMemory(conversationId: string, userId: string, organizationId = DEFAULT_ORGANIZATION_ID) {
  await getOwnedConversation(conversationId, userId, organizationId);
...
export async function exportConversation(conversationId: string, userId: string, organizationId = DEFAULT_ORGANIZATION_ID) {
  const { conversation, messages: rows } = await getConversationWithMessages(conversationId, userId, organizationId);
