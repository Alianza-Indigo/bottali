...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const rows = await db.select().from(knowledgeBases).where(eq(knowledgeBases.id, id)).limit(1);
...
export async function PATCH(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await params;
    await disableKnowledgeBase(id, user.id);
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const user = await requireUserWithPermission("knowledge.manage");
    const { id } = await params;
    await deleteKnowledgeBase(id, user.id);
