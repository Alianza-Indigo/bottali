...
const createSchema = z.object({ toolId: z.string().uuid().optional(), name: z.string().min(1).max(120), description: z.string().max(2000).optional() });
...
export async function POST(request: Request) {
  try {
    ...
    const kb = await createKnowledgeBase(body.toolId ?? null, body.name, body.description, user.id);
